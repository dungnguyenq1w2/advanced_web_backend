...
const htmlContent = (user, hashedEmail) => {
    return `<a href="${process.env.APP_FRONT_END_URL}/auth/verify?email=${user}&token=${hashedEmail}`
}

const htmlContentInviteGroup = (groupId, email, token) => {
    return `<a href="${process.env.APP_FRONT_END_URL}/group/${groupId}/invite?email=${email}&token=${token}`
}
